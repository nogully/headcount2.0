export default class DistrictRepository {
  ...
  findAllMatches(string) {
    ...
    findAverage(districtName) {
      ...
      const average =  percents.reduce( (sum, percent) => {
        sum += percent;
        return sum;
      }, 0) / percents.length
    ...
    compareDistrictAverages(dist1, dist2) {
      ...
      if (foundDistrict1 && foundDistrict2){
        comparisonObj[foundDistrict1.location] = this.findAverage(foundDistrict1.location);
        comparisonObj[foundDistrict2.location] = this.findAverage(foundDistrict2.location);
        comparisonObj.compared = Math.round((this.findAverage(dist1) / this.findAverage(dist2)) * 1000) / 1000
      }
